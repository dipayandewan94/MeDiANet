...
class ResidualBlock(nn.Module):  # mostly same as tensorflow except for bias term which is turned off here
    def __init__(self, input_channels, output_channels, stride = 1):
        ...
        self.mish = nn.Mish()
        self.conv1 = nn.Conv2d(input_channels, output_channels//4, 1)  # same as tensorflow
...
class am1(nn.Module):
    def __init__(self, in_channels, out_channels, dilation_rate = [4,8,12]):
        ...
        self.skip3_connection_residual_block = dilated_residual_block(in_channels, out_channels, dilation_rate=dilation_rate)  # added 

        self.op1 = dilated_residual_block(in_channels, out_channels, dilation_rate=dilation_rate) # added
        self.op2 = dilated_residual_block(in_channels, out_channels, dilation_rate=dilation_rate) # added
    ...
    def forward(self, x):
        ...
        out_op1 = self.op1(out_softmax3) # added
        #out_op2 = self.op2(out_op1) # added
        out_bottleneck = torch.add(out_op1, out_skip3_connection) # added
        out_op2 = self.op2(out_bottleneck)

        # Decoder
        out_interp3 = self.interpolation3(out_op2) # modded
...
class am2(nn.Module):
    def __init__(self, in_channels, out_channels, dilation_rate = [2,4,6]):
        ...
        self.mpool2 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0) 
        self.softmax2_blocks = dilated_residual_block(in_channels, out_channels,dilation_rate=dilation_rate) #nn.Sequential(dilated_residual_block(in_channels, out_channels, dilation_rate=dilation_rate), dilated_residual_block(in_channels, out_channels, dilation_rate=dilation_rate)) # 1,2
        self.skip2_connection_residual_block = dilated_residual_block(in_channels, out_channels, dilation_rate=dilation_rate) # skip2
        
        self.op0 = dilated_residual_block(in_channels, out_channels,dilation_rate=dilation_rate) # after skip calculated
        self.op1 = dilated_residual_block(in_channels, out_channels, dilation_rate=dilation_rate) # 3
        self.op2 = dilated_residual_block(in_channels, out_channels, dilation_rate=dilation_rate) # 4
...
class am3(nn.Module):
    def __init__(self, in_channels, out_channels, dilation_rate = [2,4,6]):
        ...
        self.softmax1_blocks = dilated_residual_block(in_channels, out_channels,dilation_rate=dilation_rate)  #nn.Sequential(dilated_residual_block(in_channels, out_channels,dilation_rate=dilation_rate),dilated_residual_block(in_channels, out_channels, dilation_rate=dilation_rate))
        self.skip1_connection_residual_block = dilated_residual_block(in_channels, out_channels,dilation_rate=dilation_rate)
...
class Medianet69(nn.Module):
    ...
    def __init__(self,  numclasses = 13, n_channels=32):
        ...
        #self.drb = dilated_residual_block(128,128,dilation_rate = [1,2,4])
        self.residual_block4 = ResidualBlock(4*n_channels, 8*n_channels, stride=2)    #_/
        self.residual_block5 = ResidualBlock(8*n_channels, 256)  #_/
        #self.avgpool = nn.AvgPool2d(kernel_size=4, stride=1)    #_/
        self.avgpool = nn.AvgPool2d(kernel_size=7, stride=1)  
        self.flatten = nn.Flatten() #_/
        self.dropout = nn.Dropout(0.25)
        self.dense1 = nn.Linear(256,2*n_channels) #_/
        self.mish2 = nn.Mish()
        #self.dense2 = nn.Linear(64, hashbit)
        #self.tanh = nn.Tanh()
        #self.fc1 = nn.Linear(128, 64)
        self.fc2 = nn.Linear(2*n_channels, numclasses)

        #self.residual_block6 = ResidualBlock(128,256)  # added by dyutit
...
class Medianet117(nn.Module):
    ...
    def __init__(self,  numclasses = 13, n_channels=32):
        ...
        #self.drb = dilated_residual_block(128,128,dilation_rate = [1,2,4])
        self.residual_block4 = ResidualBlock(4*n_channels, 8*n_channels, stride=2)    #_/
        self.residual_block5 = ResidualBlock(8*n_channels, 8*n_channels)  #_/
        #self.avgpool = nn.AvgPool2d(kernel_size=4, stride=1)    #_/
        self.avgpool = nn.AvgPool2d(kernel_size=7, stride=1)  
        self.flatten = nn.Flatten() #_/
        self.dense1 = nn.Linear(256,2*n_channels) #_/
        ...
        self.residual_block6 = ResidualBlock(8*n_channels,256)  # added by dyutit
