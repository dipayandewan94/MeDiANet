def train():
    ...
    with strategy.scope():
        model = MeDiANet69(shape=shape, n_channels=n_channel, n_classes=n_classes, dropout = 0.25, regularization=0.02, drop_prob=0.2)
    ...
    history = model.fit(train_data,
                ...
                shuffle = True,
                callbacks=[CSVLogger('MeDiANet69.csv'),
                            ModelCheckpoint('MeDiANet69.keras', monitor='val_acc', mode='max', verbose=1, save_best_only=True),
                            ClearMemory()])  
