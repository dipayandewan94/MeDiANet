def train():   
    ...
    #--------------------------------MODEL:--------------------------------#
    from model_pytorch import Medianet117
    #from model_torch import am3

    model = Medianet117(numclasses=35, n_channels=16)
    #n_channels = 16
    ...
    log_file = os.path.join(logs_dir, "model117_base_training.csv")
    file_exists = os.path.isfile(log_file)
    log_df = pd.DataFrame(columns=["Epoch", "Loss", "Accuracy"])
    log_df.to_csv(log_file, index=False)

    # Initializing model save:
    saves_dir = logs_dir = os.path.join(os.path.dirname(__file__), 'torch_saved_models')
    os.makedirs(logs_dir, exist_ok=True)
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        if accuracy_epoch > best_accuracy:
            best_accuracy = accuracy_epoch
            #model_save_path = "checkpt_model117.pth"
            model_save_path = os.path.join(saves_dir, "checkpt_model117.pth")
            torch.save({
            'epoch': epoch,
